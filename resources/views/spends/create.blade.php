		...
		<div class="my-4 flex justify-center">
			<div class="bg-white shadow p-6 rounded-lg w-1/2">
				<form action="{{ route('spends.store') }}" method="post">
					@csrf
					<div class="mb-4">
						<label for="user_name" class="block text-gray-700 text-sm font-bold mb-2">購入者名</label>
						<input type="text" name="user_name" id="user_name" class="w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200 focus:border-indigo-500" required>
					</div>
					<div class="mb-4">
						<label for="name" class="block text-gray-700 text-sm font-bold mb-2">支出内容</label>
					...
					<div class="flex justify-end">
						...
						<a href="{{ route('spends.index') }}" class="py-2 px-4 ml-4 btn btn-secondary">キャンセル</a>
					</div>
				</form>
			</div>
		</div>
